...
from pageObjects.LocatorsDocumentAndWorkflow import DashboardAndTabs, documentRegisterAndWorkflow
...
class Test_004_Workflow_Creation:
    ...
    def test_workflow_creation(self,setup):
        ...
        # below input is to validate the WF created in docProperties table - START
        self.docname = XLUtils.readData(self.path, "Inputs", 2, 8)
        # below input is to validate the WF created in docProperties table - END
        ...
        # kept the excel line here to have the WF name info ready until this time - DONOT MOVE
        self.wf_name = XLUtils.readData(self.path, "Inputs", 2, 14)
        self.docReg_Wf.validate_wf_properties(self.docname, self.wf_name)
        self.logger.info("** Workflow successfully validated **")
